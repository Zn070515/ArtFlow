...
VS Code Pylance must use the repository-root `pyrightconfig.json`. Before committing production Python changes, run both `npm run check:pyright` and `npm run check:pyright:entry-access`; both commands are blocking gates and must report zero errors, warnings, and informations. When Django runtime-generated fields, reverse managers, annotated queryset fields, or choice display methods are not visible to Pylance, add a narrow `TYPE_CHECKING` declaration in the owning model module. Do not globally disable `reportAttributeAccessIssue`, lower the project checking mode, or hide a new diagnostic with a broad `Any`/file-level suppression. Any new production Python module must be included by the project baseline or an explicitly documented scoped Pyright configuration.
