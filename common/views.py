...
def controlled_media(request, path):
    ...
    if PublicPost.objects.filter(cover_image=relative_path, status=PublicPost.Status.PUBLISHED).exists():
    ...
    if GeneratedDocument.objects.filter(file=relative_path).exists():
        ...
        return _media_file_response(relative_path, Path(document.file.name).name)

    if ExportTask.objects.filter(file=relative_path).exists() or ArchivePackage.objects.filter(file=relative_path).exists():
        if not request.user.is_authenticated or not request.user.is_staff_or_admin:
