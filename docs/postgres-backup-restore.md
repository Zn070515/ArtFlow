...
成功标准是：备份文件存在且非空，恢复目标可连接，Django `manage.py check` 通过，并能读取 `django_migrations`。Manifest 只记录 Ticket、Ticket access session、ballot 等数量和迁移元数据；Ticket digest 属于凭据派生数据，原始 Ticket secret/session token 不得进入 manifest、日志、导出或演练产物。失败时保留源服务和卷，记录失败原因后重新演练。
