...
1. 新机器只需安装 Python 3.12、uv 和 Docker，即可创建环境并启动本地 Django + PostgreSQL。
2. `uv.lock` 是依赖解析的冻结依据，`pyproject.toml` 是项目和工具配置的单一来源。
3. 一条命令可以完成迁移、配置检查、测试数据初始化和健康检查。
...
`requirements.txt` 不再作为手工依赖来源；如果保留，则由 `uv export` 生成并在文档中标记为兼容产物，避免两个依赖清单漂移。
...
- 所有演示业务数据显式标记为测试数据，覆盖公开门户、歌手比赛、毕晚、评分、投票、导出模板和审计场景。
- 默认不删除任何数据。
- `--reset` 只能删除由该命令创建且带有测试标记的数据；正式数据必须保持不变。
- 管理员密码只能从 `DEV_ADMIN_PASSWORD` 或显式参数读取，不提供仓库内默认密码。
...
uv run mypy config accounts core common public_portal files singer_contest farewell_show voting exports archive incidents staff_panel
uv run python manage.py check
uv run python manage.py makemigrations --check --dry-run
uv run pytest -q
```

`scripts/verify.ps1` 另外以生产语义执行 `manage.py check --deploy`，使用临时的非真实密钥和主机配置，不把生产默认值写入仓库。pytest 通过 `pytest-django` 收集现有 `tests.py`，启用严格 marker 和 warnings-as-errors；初始覆盖率门槛以基线实测值为准，并设为只能上调的门槛。Ruff 先覆盖项目源代码、配置、脚本和测试，排除生成迁移文件中的机械格式噪音；排除项必须写入配置并说明原因。

mypy 采用渐进式强类型门禁：新建和修改的基础设施模块（`config`、`common`、`scripts`、management commands）使用 strict；既有未类型化业务视图先启用 `check_untyped_defs`，不得借机大规模改写业务。每个被修改的模块都必须逐步移入 strict 范围。
...
- `web`：ArtFlow 镜像，等待数据库就绪后执行迁移，再启动 Django 服务。
- `media`：独立媒体卷；宿主机不把真实上传文件写入 Git 工作区。
- 健康检查调用 `/healthz/`，容器异常时返回非零状态。

容器启动脚本必须使用 `exec` 传递最终进程，迁移失败时立即退出，不能在错误状态下启动 Web 服务。生产镜像不包含测试数据、开发密钥或源码控制目录。

## CI 分层

### CI

Linux job 使用 uv lock，在 SQLite 上运行 Ruff、mypy、Django checks、迁移检查和 pytest，并上传 coverage。Windows job 运行 Python 3.12 和 3.13，验证 Windows 路径、上传文件和模板渲染行为。所有 job 设有超时、最小权限和 concurrency 取消旧运行。

### Integration

使用 PostgreSQL service 或 Docker Compose 执行迁移、`doctor`、`healthz` 和完整测试，并验证 `seed_demo_data` 幂等性。集成 job 不使用生产凭据。
...
6. 更新 README、CLAUDE、AGENTS、开发手册和变更记录，删除所有 `seed_data`、不存在的 Docker 命令和过时的模型统计。
