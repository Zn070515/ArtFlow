...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument("source_dir", help="Directory containing one subfolder per activity/gallery.")
        parser.add_argument(
            "--publish",
            action="store_true",
            help="Publish imported public posts immediately. Internal folders stay draft unless --include-internal is set.",
        )
        parser.add_argument(
            "--include-internal",
            action="store_true",
    ...
    def handle(self, *args, **options):
        ...
        for folder in sorted([p for p in source_dir.iterdir() if p.is_dir()], key=lambda p: p.name):
            ...
            image_paths = [
                p for p in sorted(folder.rglob("*"), key=lambda p: p.name)
                if p.is_file() and p.suffix.lower() in IMAGE_EXTENSIONS
            ]
            if not image_paths:
                continue

            status = PublicPost.Status.PUBLISHED if options["publish"] and not is_internal else PublicPost.Status.DRAFT
            post, created = PublicPost.objects.get_or_create(
                ...
                defaults={
                    ...
                    "status": status,
                    "published_at": timezone.now() if status == PublicPost.Status.PUBLISHED else None,
                },
            )
            if created:
                imported_posts += 1
            elif options["publish"] and not is_internal and post.status != PublicPost.Status.PUBLISHED:
        ...
        self.stdout.write(self.style.SUCCESS(
            f"Imported {imported_media} media item(s), {imported_posts} post(s); skipped {skipped_media} existing item(s)."
        ))
