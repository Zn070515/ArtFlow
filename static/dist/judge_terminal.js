...
(() => {
    ...
    function parseContext(value) {
        ...
        if (!positiveInteger(candidate.activity_id) ||
            ...
            return null;
        const criteria = candidate.rubric_payload.criteria.flatMap((item) => {
            if (!isRecord(item) || typeof item.name !== "string" ||
                typeof item.max_score !== "string" || !nonNegativeInteger(item.sequence))
                return [];
            return [{ name: item.name, max_score: item.max_score, sequence: item.sequence }];
        });
        if (criteria.length !== candidate.rubric_payload.criteria.length)
            return null;
        return {
    ...
    function readDraft(fingerprint) {
        ...
        try {
            ...
            if (!isRecord(value) || typeof value.command_id !== "string" ||
                ...
                value.score_payload.notes.length > 200 || !validScore(value.score_payload.score))
                return null;
            return {
    ...
    async function boot() {
        ...
        try {
            ...
            if (draft) {
                ...
                if (notesField)
                    notesField.value = draft.score_payload.notes;
            }
